...
def test_plot_ts(tmpdir):
    # define col_names as empty list because the plot_ts functions needs
    # it as input argument
    col_names = []
    ...
    fig = graphics.plot_ts(df, col_names, tmpdir)
    ...
    graphics.plot_ts(df, col_names ,filepath=fpath)
    assert os.path.exists(fpath)
